...
derived exactly as specified: \texttt{available\_inventory = stock\_on\_hand +
replenishment}; \texttt{units\_fulfilled = min(available\_inventory, units\_demanded)};
\texttt{unmet\_demand = max(units\_demanded - available\_inventory, 0)};
\texttt{ending\_stock = max(available\_inventory - units\_demanded, 0)}; and
\texttt{stockout = 1} if \texttt{unmet\_demand > 0}. Reconciliation
(demand = fulfilled + unmet) and the week-to-week stock-chaining property were both verified
...
agreement breaks down. On 12 of 24 parts, the most statistically accurate method was
\emph{not} the cheapest one -- choosing accuracy over cost on those parts costs \textbf{33.5\%
more on average}, peaking at \textbf{152.9\% more} on part P023 (Figure~\ref{fig:disagreement}).

\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{disagreement.png}
...
On these parts, Holt's slightly stronger under-forecasting bias is punished disproportionately
by the high per-unit stockout cost, even though its average error (MAE) was smaller than the
alternative method's.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{segmentation.png}
  \caption{Parts segmented by demand volume and unit cost (median split), colored by
  historical stockout cost. The expensive/high-demand quadrant (P002, P004, P008, P011, P018)
  carries the largest stockout-cost exposure.}
...
\begin{enumerate}[nosep]
  ...
  optimized per part; and the reorder policy has not yet been tested against future
  (out-of-sample) data.
\end{enumerate}
...
\midrule
[Name 1] & [e.g., data audit, merge, derived variables; problem formulation] \\
[Name 2] & [e.g., EDA, demand classification, segmentation, visualization] \\
[Name 3] & [e.g., forecasting methods, walk-forward engine, dual evaluation] \\
\bottomrule
\end{tabular}
\end{center}
...
P022 & exp\_smoothing  & moving\_average& 2{,}552.1  & 2{,}340.5 & 9.0   \\
P005 & holt            & exp\_smoothing & 5{,}555.6  & 5{,}281.0 & 5.2   \\
\bottomrule
\end{tabular}
\end{center}

\noindent Average premium across all 12 disagreeing parts: \textbf{33.5\%}.
